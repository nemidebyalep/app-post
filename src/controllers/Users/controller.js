...
userController.register = async(req, res) => {
    try {
        ...
        const newUser = await pool.query('INSERT INTO usuarios( username, password) values($1,$2) RETURNING *', [username, passwordEncrypted]);
...
router.get('/admin', verifyToken, async(res) => {
    try {
        res.json({ Bienvenida: 'Bienvenid@' });
    } catch (error) {
